...
-- danny change dataset
-- local trainset, validset, testset = dl.loadPTB({opt.batchsize,1,1})
local trainset, validset, testset = dl.loadPTB(opt.batchsize,'./data/','https://raw.githubusercontent.com/eladhoffer/recurrent.torch/master/examples/language/data/')
if not opt.silent then 
...
if not opt.silent then
   ...
   -- Optimization configuration
	local Weights,Gradients = lm:getParameters()
   print("Num of parameters:")
   print(Weights:nElement())
end
...
while opt.maxepoch <= 0 or epoch <= opt.maxepoch do
   ...
   trainLoss[epoch] = sumErr
   trainPerplexity[epoch] = ppl
   ...
   validLoss[epoch] = sumErr
   validPerplexity[epoch] = ppl
   
   -- Danny - testing
   lm:evaluate()
   local sumErr = 0
   for i, inputs, targets in testset:subiter(opt.seqlen, testsize) do
      targets = targetmodule:forward(targets)
      local outputs = lm:forward(inputs)
      local err = criterion:forward(outputs, targets)
      sumErr = sumErr + err
   ...
   local testppl = torch.exp(sumErr/testsize)
   -- Perplexity = exp( sum ( NLL ) / #w)
   print("Test PPL : "..testppl)

   xplog.testppl[epoch] = testppl
   
   testLoss[epoch] = sumErr
   testPerplexity[epoch] = testppl
